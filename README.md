...
graph TD
    ...
    subgraph Backend["FastAPI Backend - Port 8000"]
        TF["TelemetryFeeder\n10 Hz CSV replay loop"]
        SM["SentryMatrix\nEPI / ARF / TRM / RSI / ZScore"]
        VE["VoiceEngine\nOllama + Edge TTS + anti-chatter"]
        BM["BaselineManager\nNASA OSDR Profiles per crew"]
        DB[("SQLite WAL\nastronaut_health.db")]
    end

    Ollama["Ollama AI Server - Port 11434\nllama3.2:1b - JARVIS persona\nGenerates advisories in under 3 seconds"]

    Browser --> CG & ECG & JC
    CG & ECG -->|"WebSocket /ws/telemetry"| TF
    JC -->|"REST /api/*"| VE
    TF --> SM --> VE
    SM --> DB
    VE --> BM
...
**B. Pull the base model**

```bash
ollama pull llama3.2:1b
```

> This downloads approximately 800 MB. Only required once.

**C. Create the JARVIS persona**
...
- Calm, direct clinical language calibrated for crew under stress
...
Terminal 2 - Ollama AI (auto-starts on install, or run manually):
  ollama serve
...
data/astronaut_telemetry_stream.csv
  ...
  SentryMatrixEngine.evaluate_state()
    ...
    +-- RSI  : Radiation Sickness      (flux + Andrews lymphocyte depletion model)
         |
         v  Alert coalescing (100ms temporal window)
  WebSocket broadcast to all connected HUD clients
```
...
|---|---|---|
| **CRITICAL** | SpO2 < 90%, CO2 >= 4 mmHg, QTc >= 485ms, TRM >= 2.2, RSI >= 1.8 | Klaxon + red pulsing border |
| **WARNING** | K+ < 3.8, EPI >= 0.9, HRV Z <= -2 sigma, CO2 >= 3 mmHg, RSI >= 1.0 | Chime + amber border |
| **NOMINAL** | All parameters within personal baseline range | No alert |

Each astronaut has individual baselines derived from NASA OSDR spaceflight data, segmented by activity state: REST, WORKOUT, or SLEEP.
...
AudioService (frontend)
         ...
         +-- 3.5s mandatory quiet break, then next advisory stage
```
...
|---|---|---|
| **Hypoxia** | SpO2 drops to 86-88% | Emergency oxygen protocol |
| **CO2 Toxicity** | Cabin CO2 rises to 4.5+ mmHg | Scrubber evacuation alert |
...
+-- nasa_osdr/
    ...
    +-- OSD-575_Immune_Panel.csv                <- 143 columns, 71 cytokines (IL-6, TNF-a...)
```
...
| `cabin_co2` | mmHg | Cabin partial pressure CO2 |
| `potassium` | mmol/L | Serum electrolyte (K+) |
| `hematocrit` | % | Red blood cell volume fraction |
| `wbc_count` | k/uL | White blood cell count |
| `il_6` | pg/mL | Interleukin-6 (inflammation/sepsis marker) |
| `platelet_count` | k/uL | Thrombocyte count |
| `radiation_flux` | mGy/h | Physical dosimeter flux |
| `lymphocyte_count` | k/uL | Absolute lymphocyte count (radiation biodosimetry) |
...
| `POST` | `/api/scenario` | Trigger a scenario: `{"scenario": "HYPOXIA"}` |
...
- `tna` + `ifng` viral reactivation index: Immune panel already in `/data`
